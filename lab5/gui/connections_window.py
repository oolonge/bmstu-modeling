...
class ConnectionsCanvas(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setMinimumSize(CONNECTIONS_WINDOW_WIDTH - 100, CONNECTIONS_WINDOW_HEIGHT - 150)
    ...
    def draw_operator(self, painter: QPainter, operator_id: int):
        ...
        painter.drawText(rect, Qt.AlignCenter, f"Оп. {operator_id + 1}")
    ...
    def draw_computer(self, painter: QPainter, computer_id: int):
        ...
        font = QFont("Arial", 11, QFont.Bold)
        painter.setFont(font)
        painter.drawText(rect, Qt.AlignCenter, f"ПК {computer_id + 1}")
...
class ConnectionsWindow(QDialog):
    ...
    def init_ui(self):
        ...
        # Заголовок
        title = QLabel("Настройка связей: Операторы → Компьютеры")
        title_font = QFont()
